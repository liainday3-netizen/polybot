...
class CapitalProjectionEngine:
    ...
    async def _fetch(self) -> Optional[MarketContext]:
        ...
        try:
            ...
            btc_price  = float(data.get("bitcoin", {}).get("usd", 0))
            btc_change = float(data.get("bitcoin", {}).get("usd_24h_change", 0))
            eth_price  = float(data.get("ethereum", {}).get("usd", 0))
            eth_change = float(data.get("ethereum", {}).get("usd_24h_change", 0))
            matic_price = float(data.get("matic-network", {}).get("usd", 0))
